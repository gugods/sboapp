...
import { createStockIn, getStockInProduct, getStockInQrcode, getStockInSearch } from '../services/stockInService';
...
class StockIn extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      refreshing: false,
      fieldFilter: '',
      options: [{ value: 'IN-02', label: t('TEXT_IN-02') }],
    };
  }
  ...
  async componentDidMount() {
    ...
    if (app_roles_id === 1) {
      // isAdmin
      const options = [
        { value: 'IN-01', label: t('TEXT_IN-01') },
        { value: 'IN-02', label: t('TEXT_IN-02') },
        { value: 'IN-03', label: t('TEXT_IN-03') },
  ...
  async getStockSearch() {
    ...
    const results = await getStockInSearch(client, { branchs_id, stock_date });
  ...
  async onSelectedProduct(ids_product) {
    ...
    if (!find(stockList.data, { products_id })) {
      stockList.loading = true;
      this.setState({ stockList });
      const results = await getStockInProduct(client, { branchs_id, products_id, stock_date });
    ...
    if (isRequired) {
      ...
      items.forEach((value, index) => {
        if (value.products_id === products_id) {
          ...
          //   uri: value.stock_doc_file ? value.stock_doc_file : null,
          //   name: value.stock_doc_file ? `stock_doc_file-${index}.jpg` : null,
          //   type: value.stock_doc_file ? 'image/jpeg' : null
          // });
          products.push({
      ...
      const results = await createStockIn(client, { branchs_id, products_id, products, file });
      if (results.status) {
        const { stockList } = this.state;
  ...
  async onBackQrcode({ qrcode }) {
    ...
    const results = await getStockInQrcode(client, { branchs_id, qrcode, stock_date });
                    ...
                    <React.Fragment>
                      <View style={{ flexDirection: 'row', flex: 2, paddingLeft: 15 }}>
                        ...
                        <Table.Input
                          ...
                          style={{ width: '90%', marginLeft: 10, marginRight: 5 }}
                        />
                      </View>
                      ...
                      { value.stock_doc_file ? (
                        <TouchableOpacity onPress={() => this.onPreviewImage(index, true)}>
                          ...
                          <ImageView imageIndex={0} isVisible={!!value.isVisible} images={[{source: {uri: value.stock_doc_file}}]} style={{width: 50, height: 50}} onClose={() => this.onPreviewImage(index, false)} />
                        </TouchableOpacity>
                      ) : (
  ...
  addFilter(value) {
    ...
    value.stock_balance = value.stock_amount + value.stock_receive;

    // filter
    value.filter_amount = filterSumTotal({ count: value.stock_amount, isFilter: OVER_ZERO });
    value.filter_receive = filterSumTotal({ count: value.stock_receive, isFilter: OVER_ZERO });
    value.filter_balance = filterSumTotal({ count: value.stock_balance, isFilter: OVER_ZERO });
  }

  renderItem(value, index) {
    this.addFilter(value);
    const textStyle = value.stock_receive > 0 ? { color: GLOBALS.COLOR_SUCCESS } : {};
  ...
  renderTotal() {
    ...
    if (stockList.data.length > 0) {
      ...
      stockList.data.forEach((value, index) => {
        value.stock_receive = countItem({ items, field: 'products_id', data: value.products_id });
        total_balance += value.stock_amount + value.stock_receive;
        total_amount += value.stock_amount;
        total_receive += value.stock_receive;
      });

      const textStyle = total_receive > 0 ? { color: GLOBALS.COLOR_SUCCESS } : {};
      const amountActive = filterTotalActive('amount', fieldFilter);
      const receiveActive = filterTotalActive('receive', fieldFilter);
      ...
      return (
        <Table.Thead style={{ marginTop: 5 }}>
          ...
          <Table.Link
            onPress={() => this.handlefilterSumTotal('receive')}
            style={{ width: '18%', alignItems: 'flex-end', ...receiveActive.style }}
            textStyle={{ ...textStyle, ...receiveActive.textStyle }}
          >
            {' '}
            {total_receive}{' '}
          </Table.Link>
...
export default withApollo(withUser(withDialog(withTranslation()(StockIn))));
