// src/pages/StockOuting/StockItem.js
...
import { COMPLETED, PENDING, PENDING_SALES, PENDING_STORE } from '../../libraries/Constant';
import { countItem } from '../../libraries/Array';
import { createFilter } from 'react-native-search-filter';
import { createStockOuting, getStockOutingProduct } from '../../services/stockOutingService';
...
class StockItem extends Component {
  ...
  async componentDidMount() {
    await this.getStockOutingProduct();
  ...
  async getStockOutingProduct() {
    ...
    const results = await getStockOutingProduct(client, { branchs_id, stock_date, stock_type, stock_doc_no });
  ...
  async onSave() {
    ...
    const results = await createStockOuting(client, { branchs_id, products });
  ...
  renderItem(value, index) {
    ...
    if (value.stock_status === PENDING) {
      ...
      checkedStore = false;
      disabled = false;
    } else if (value.stock_status === PENDING_SALES) {
      checkedAdmin = false;
      checkedStore = false;
      disabled = true;
    } else if (value.stock_status === PENDING_STORE) {
      checkedAdmin = true;
      checkedStore = false;
      disabled = true;
    } else if (value.stock_status === COMPLETED) {
      checkedAdmin = true;
      checkedStore = true;
      disabled = true;
    }

    const txtStyle = disabled ? { fontSize: 10, color: GLOBALS.COLOR_DESC } : { fontSize: 10, color: GLOBALS.COLOR_MAIN };
    ...
    return (
      <Table.Tbody key={index} rowColor={index} style={bgstyle}>
        <Table.TD style={{ width: '37%' }} textStyle={txtStyle}>
          {value.product_code} : {value.product_name}
        </Table.TD>
        <Table.Checkbox
          disabled={disabled}
          style={{ width: '10%', alignItems: 'center', marginTop: -5 }}
          checked={checked}
          onChange={() => this.onCheckProductItem(index)}
        />
        <Table.Checkbox disabled={true} style={{ width: '10%', alignItems: 'center', marginTop: -5 }} checked={checkedAdmin} />
        <Table.Checkbox disabled={true} style={{ width: '10%', alignItems: 'center', marginTop: -5 }} checked={checkedStore} />
        ...
        <Table.TD style={{ width: '10%', alignItems: 'flex-end' }} textStyle={txtStyle}>
          {value.stock_amount}
        </Table.TD>
      </Table.Tbody>
    );
    ...
    return (
      <React.Fragment>
        <View style={styles.container}>
          ...
          <Table>
            <Table.Thead>
              <Table.TH style={{ width: '35%' }} textStyle={{ fontSize: 12 }}>
                <Icon name='time' size={18} /> {t('TEXT_PRODUCT')}
              </Table.TH>
              <Table.TH style={{ width: '10%', alignItems: 'flex-end' }} textStyle={{ fontSize: 10 }}>
                PC
              </Table.TH>
              <Table.TH style={{ width: '10%', alignItems: 'flex-end' }} textStyle={{ fontSize: 10 }}>
                KEY
              ...
              <Table.Component style={{ width: '23%', alignItems: 'flex-start' }} textStyle={{ fontSize: 10 }}>
                <Text style={styles.btnDate}>{t('TEXT_DATE_OUT')}</Text>
                <InputDateText date={stock_date_all} hide={true} onDateChange={this.onDateChange.bind(this, null)} />
              </Table.Component>
              <Table.TH style={{ width: '12%', alignItems: 'flex-end' }} textStyle={{ fontSize: 10 }}>
                {t('TEXT_AMOUNT')}
              </Table.TH>
            </Table.Thead>
            {this.renderStockOuting()}
            {this.renderTotal()}
          </Table>
